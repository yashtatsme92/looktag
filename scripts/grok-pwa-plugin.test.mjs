...
test("look and house paths are share documents", () => {
  assert.equal(isShareDocumentPath("/looks/seed-sunday-coat"), true);
  assert.equal(isShareDocumentPath("/houses/label-atelier-noir"), true);
  assert.equal(isShareDocumentPath("/houses/label-atelier-noir/kinkistyles"), true);
  assert.equal(isShareDocumentPath("/looks/seed-sunday-coat/edit"), false);
  assert.equal(isShareDocumentPath("/"), false);
});

test("keeps look-specific Open Graph tags on anonymous share documents", () => {
  const html = `<html><head><title>Sunday Coat — Looktag</title><meta name="looktag:share" content="look"><meta property="og:title" content="Sunday Coat — Looktag"><meta property="og:url" content="https://looktag.example/looks/seed-sunday-coat"><meta property="og:image" content="https://looktag.example/looks/sunday-coat.jpg"></head><body></body></html>`;
  const out = injectGrokPwaHead(html, {
    pathname: "/looks/seed-sunday-coat",
    host: "looktag.example",
    site: { title: "Looktag", card: "custom" },
  });
  assert.match(out, /property="og:image" content="https:\/\/looktag\.example\/looks\/sunday-coat\.jpg"/);
  assert.match(out, /property="og:title" content="Sunday Coat — Looktag"/);
  assert.doesNotMatch(out, /property="og:title" content="Looktag"/);
  assert.match(out, /grok-app-builder\/extensions\.js/);
});

test("keeps collection Open Graph tags on anonymous share documents", () => {
  const html = `<html><head><title>Kinkistyles — Atelier Noir</title><meta name="looktag:share" content="collection"><meta property="og:title" content="Kinkistyles — Atelier Noir"><meta property="og:url" content="https://looktag.example/houses/label-atelier-noir/kinkistyles"></head><body></body></html>`;
  const out = injectGrokPwaHead(html, {
    pathname: "/houses/label-atelier-noir/kinkistyles",
    host: "looktag.example",
    site: { title: "Looktag", card: "custom" },
  });
  assert.match(out, /property="og:title" content="Kinkistyles — Atelier Noir"/);
  assert.doesNotMatch(out, /property="og:title" content="Looktag"/);
});

test("still stamps the site card when a share document has no page meta", () => {
  const out = injectGrokPwaHead("<html><head><title>Sunday Coat — Looktag</title></head></html>", {
    pathname: "/looks/seed-sunday-coat",
    host: "looktag.example",
    site: { title: "Looktag" },
  });
  assert.match(out, /property="og:title" content="Looktag"/);
});
